...
add_action( 'admin_post_murailles_save_options', function () {
	...
	// ── Currency exchange rates (1 unit = X MAD) ─────────────────────────────
	foreach ( array( 'rate_eur', 'rate_usd' ) as $rate_field ) {
		if ( isset( $post[ $rate_field ] ) ) {
			$rate = (float) str_replace( ',', '.', sanitize_text_field( $post[ $rate_field ] ) );
			$clean[ $rate_field ] = $rate > 0 ? $rate : '';
		}
	}
	if ( isset( $clean['currency_default'] ) && ! array_key_exists( $clean['currency_default'], murailles_get_currencies() ) ) {
		$clean['currency_default'] = 'MAD';
	}
...
function murailles_options_page() {
	...
	$nav = array(
		...
		'currency'     => array( '💱', 'Devises',         'MAD / EUR / USD et taux de change' ),
	...
	<div class="wrap murailles-options-wrap murailles-app">
		...
		<div class="mi-shell">
			...
			<?php
			// ── Tab: Devises / Currency ───────────────────────────────────────
			if ( $tab === 'currency' ) :
				$cur_default = $o( 'currency_default', 'MAD' );
			?>
			<div class="mi-card">
				<div class="mi-card-head"><span class="mi-card-ico">💱</span><h3>Devises & taux de change</h3><span class="mi-card-sub">MAD / EUR / USD</span></div>
				<div class="mi-card-body">
					<p class="mi-hint" style="margin-bottom:16px;">
						Chaque bien a sa propre devise de base (choisie dans l'édition du bien). Sur le site, un sélecteur permet aux visiteurs de convertir les prix en MAD, EUR ou USD. Renseignez ici la valeur en dirhams (MAD) d'<strong>1 EUR</strong> et d'<strong>1 USD</strong>.
					</p>
					<div class="mi-field">
						<label>Devise par défaut</label>
						<select name="currency_default">
							<?php foreach ( murailles_get_currencies() as $cur_code => $cur_data ) : ?>
							<option value="<?php echo esc_attr( $cur_code ); ?>" <?php selected( $cur_default, $cur_code ); ?>><?php echo esc_html( $cur_data['label'] ); ?></option>
							<?php endforeach; ?>
						</select>
						<span class="mi-hint">Utilisée pour les biens sans devise enregistrée et comme devise affichée par défaut.</span>
					</div>
					<div class="mi-grid-2">
						<div class="mi-field">
							<label>1 EUR = ? MAD</label>
							<input type="text" name="rate_eur" value="<?php echo $o('rate_eur','10.8'); ?>" placeholder="10.8">
							<span class="mi-hint">Ex : 10.8 signifie 1 € = 10,80 MAD.</span>
						</div>
						<div class="mi-field">
							<label>1 USD = ? MAD</label>
							<input type="text" name="rate_usd" value="<?php echo $o('rate_usd','10.0'); ?>" placeholder="10.0">
							<span class="mi-hint">Ex : 10 signifie 1 $ = 10,00 MAD.</span>
						</div>
					</div>
				</div>
			</div>
