				...
				<div class="container">
					<div class="row align-items-center">
						...
						<div class="col-lg-4 col-md-5 col-sm-12 pl-1">
							<?php
							/* Only render thumbnail tiles for gallery images that
							   actually exist. Missing slots are hidden entirely (no
							   broken placeholders): a bien with fewer than 4 photos
							   simply shows fewer tiles. */
							$mu_thumb_shown = 0;
							for ( $i = 1; $i <= 3; $i++ ) :
								if ( ! isset( $img_ids[ $i ] ) ) {
									continue;
								}
								$gimg  = wp_get_attachment_image_url( $img_ids[ $i ], 'medium_large' );
								$gfull = wp_get_attachment_image_url( $img_ids[ $i ], 'large' );
								if ( ! $gimg ) {
									continue;
								}
								$mt = $mu_thumb_shown > 0 ? ' mt-3' : '';
								$mu_thumb_shown++;
							?>
							<div class="gg_single_part-right min<?php echo $mt; ?>">
								<a href="<?php echo esc_url( $gfull ?: $gimg ); ?>" class="murailles-lightbox h-100"><img src="<?php echo esc_url( $gimg ); ?>" class="img-fluid full-width rounded object-fit h-100" alt="<?php echo esc_attr( get_the_title( $id ) ); ?>" /></a>
							</div>
							<?php endfor; ?>
						</div>
					</div>
							...
							<div class="property-sidebar">
								...
								<div class="sider_blocks_wrap">
									<div class="side-booking-header">
										<div class="sb-header-left"><h3 class="price"><?php echo murailles_price_html( $id ); ?><?php if ( $suffix ) : ?><sub>/<?php echo esc_html( $suffix ); ?></sub><?php endif; ?></h3></div>
										<div class="price_offer bg-seegreen"><?php echo esc_html( $action ); ?></div>
									</div>
									<?php murailles_currency_switcher(); ?>
											...
											<div class="sides_list_property_detail">
												...
												<div class="lists_property_price">
													...
													<div class="lists_property_price_value">
														<h4><?php echo murailles_price_html( $sp_id ); ?></h4>
													</div>
												</div>
											</div>
										</div>
