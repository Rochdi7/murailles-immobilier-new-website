...
**Contrast values verified (WCAG AA, ≥4.5:1 for normal text):**
- Global body text `#72809D`→`#5e6a82` on `#ffffff` → **5.44:1** ✅ (fixes section descriptions, "Comment ça marche" steps, about-us paragraphs, testimonial copy, "Aucun bien disponible", blog intro — all inherited the failing `body` colour)
- `.text-muted` `#5b6678` on `#ffffff` → **5.81:1** ✅
- Footer `#9aa6bd` on `#1d2636` → **6.19:1** ✅
- Rating badge "4.5" — orange `#ff9800`→`#b25e00`, white text → **4.67:1** ✅

### Slick slider ARIA (mobile audit failures)
`assets/js/custom.js` adds `muraillesSanitizeSlickAria()` — runs after every
Slick init/reInit/afterChange/setPosition and on load. It removes the invalid
ARIA Slick injects:
- `.slick-dots` `role="tablist"` + `<li role="presentation">` carrying
  `aria-selected`/`aria-controls` (presentation-role conflict, invalid values,
  tablist without tab children) → reverted to native list; dot buttons get an
  `aria-label` + valid `aria-current`.
- `.slick-track` `role="listbox"` + slide `role`/`aria-describedby` (options
  without accessible names) → removed.

Attributes only — no DOM nodes, handlers, swipe, or keyboard nav changed.
