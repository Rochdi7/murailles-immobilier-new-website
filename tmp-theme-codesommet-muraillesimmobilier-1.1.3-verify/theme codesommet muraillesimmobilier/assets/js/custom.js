...
$(function() {
	...
	(jQuery, window, document), $(document).ready(function() {
		if ( typeof $.fn.navigation === 'function' && $("#navigation").length ) {
			...
			// Detach the bundled plugin's own click/touchstart bindings on the
			// burger, ✕ close button and overlay so our single guarded handlers
			// (below) are the ONLY controllers — prevents the open/close fight
			// that made the mobile menu behave erratically.
			$('#navigation').find('.nav-toggle, .nav-menus-wrapper-close-button, .nav-overlay-panel').off('click touchstart');
		}
	});
	...
	/* Sort dropdown ↔ currency switcher overlap guard.
	   The sort dropdown (Popper-positioned, own transform/stacking context)
	   can overlap the currency switcher row below it. z-index alone proved
	   unreliable across these separate stacking contexts, so while the
	   dropdown is open we hide the switcher with an INLINE style (immune to
	   CSS caching) and restore it on close. */
	(function () {
		var dd = document.querySelector('.shorting-right .dropdown');
		if ( ! dd ) {
			return;
		}
		function switchers() {
			return document.querySelectorAll('.murailles-currency-switcher.is-listing');
		}
		function setHidden( hidden ) {
			var list = switchers();
			for ( var i = 0; i < list.length; i++ ) {
				list[ i ].style.visibility = hidden ? 'hidden' : '';
			}
		}
		function isOpen() {
			if ( dd.classList.contains('show') ) { return true; }
			var menu = dd.querySelector('.dropdown-menu');
			return !! ( menu && menu.classList.contains('show') );
		}
		function sync() { setHidden( isOpen() ); }

		// Bootstrap events (fast path) + class observer (covers manual toggles).
		$(document).on('shown.bs.dropdown hidden.bs.dropdown', '.shorting-right .dropdown', sync);
		if ( typeof MutationObserver !== 'undefined' ) {
			new MutationObserver( sync ).observe( dd, { attributes: true, attributeFilter: ['class'], subtree: true } );
		}
		sync();
	})();
	...
	/* ------------------------------------------------------------------
	 * Slick ARIA sanitizer (Lighthouse Accessibility)
	 * ------------------------------------------------------------------
	 * Slick injects invalid ARIA that fails the audit:
	 *   • .slick-dots <ul role="tablist"> with <li role="presentation"> that
	 *     ALSO carry aria-selected / aria-controls (presentation-role conflict
	 *     + tablist missing real tab children + invalid values).
	 *   • .slick-track role="listbox" whose option slides have no accessible
	 *     name (ARIA input field without a name).
	 * These roles add nothing for users (the dots are decorative, the track is
	 * a plain carousel) but break the a11y tree. We strip them after every
	 * Slick (re)render. Slider behaviour, keyboard arrows, and swipe are
	 * untouched — we only remove ARIA attributes, never DOM nodes or handlers.
	 * ---------------------------------------------------------------------- */
	function muraillesSanitizeSlickAria(context) {
		var $ctx = context ? $(context) : $(document);

		// Dots: drop the tablist/presentation/state attributes so the <ul>/<li>
		// fall back to native list semantics (valid + harmless).
		$ctx.find('.slick-dots').removeAttr('role');
		...
		// Track / slides: remove the listbox/option roles that require
		// accessible names we can't meaningfully provide on a content carousel.
		$ctx.find('.slick-track').removeAttr('role');
		$ctx.find('.slick-slide')
			.removeAttr('role')
			.removeAttr('aria-describedby');
	}

	// Re-sanitize on every Slick lifecycle event that re-writes ARIA.
	$(document).on(
		'init reInit afterChange setPosition',
		'.slick-initialized, .modern-testimonial, .list_views, .item-slide-2, .testi-slide, .home-slider',
		function() { muraillesSanitizeSlickAria(this); }
	);
	// First pass for sliders already initialized before these handlers bound,
	// plus a load-time pass to catch late inits.
	muraillesSanitizeSlickAria(document);
	$(window).on('load', function() { muraillesSanitizeSlickAria(document); });
