...
( function () {
	...
	function fallbackThumb() {
		return 'data:image/svg+xml;utf8,' + encodeURIComponent(
			'<svg xmlns="http://www.w3.org/2000/svg" width="400" height="200" viewBox="0 0 400 200"><rect width="400" height="200" fill="#e9ecef"/><text x="200" y="105" text-anchor="middle" fill="#adb5bd" font-family="Arial" font-size="14">Aucune image disponible</text></svg>'
		);
	}
	...
	function render() {
		...
		var fb = fallbackThumb();
		listEl.innerHTML = list.map( function ( p ) {
			var thumb = ( p.thumb && p.thumb.length > 4 ) ? p.thumb : fb;
			var title = p.title || 'Bien sans titre';
			var url   = p.url && p.url !== '#' ? p.url : '#';
			// If the stored thumbnail URL is broken, swap to the placeholder
			// (onerror nulls itself so a broken placeholder can't loop).
			var onerr = 'this.onerror=null;this.src=\'' + fb + '\';';
			return '<div class="col-lg-4 col-md-6 col-sm-12" data-murailles-id="' + p.id + '">'
				+ '<div class="property-listing">'
					+ '<div class="listing-img-wrapper">'
						...
						+ '<a href="' + escapeHtml( url ) + '"><img src="' + escapeHtml( thumb ) + '" alt="" onerror="' + escapeHtml( onerr ) + '"></a>'
					+ '</div>'
