		...
		<div class="row">
			<div class="col-lg-12 col-md-12 position-relative z-1">
				...
				<div class="faq-search">
					<form method="get" action="#" data-murailles-faq-search role="search" onsubmit="return false;">
						<input type="search" id="murailles-faq-q" name="q" class="form-control" autocomplete="off" placeholder="<?php echo esc_attr( murailles_t( 'Recherchez une question (achat, location, frais…)', false ) ); ?>" aria-label="<?php echo esc_attr( murailles_t( 'Rechercher une question', false ) ); ?>">
...
<!-- Live-search "no results" message (hidden by default) -->
<div class="faq-no-results container" data-murailles-faq-empty hidden style="text-align:center;padding:0 0 40px;color:#6c757d;">
	<i class="ti-search" aria-hidden="true" style="font-size:32px;display:block;margin-bottom:10px;color:#cbd3dd;"></i>
	<p style="margin:0;"><?php echo esc_html( murailles_t( 'Aucune question ne correspond à votre recherche.', false ) ); ?></p>
</div>

<script>
/* FAQ live search — filters questions/answers client-side, no page reload. */
( function () {
	var form  = document.querySelector( '[data-murailles-faq-search]' );
	var input = document.getElementById( 'murailles-faq-q' );
	if ( ! form || ! input ) { return; }

	var sections = Array.prototype.slice.call( document.querySelectorAll( '.faq_wrap' ) );
	var empty    = document.querySelector( '[data-murailles-faq-empty]' );

	function normalize( s ) {
		// Lowercase + strip accents so "etranger" matches "étranger", etc.
		return ( s || '' ).toLowerCase().normalize( 'NFD' ).replace( /[̀-ͯ]/g, '' );
	}

	// Cache each card with its normalized searchable text once.
	var cards = [];
	sections.forEach( function ( section ) {
		Array.prototype.slice.call( section.querySelectorAll( '.card' ) ).forEach( function ( card ) {
			cards.push( {
				el:      card,
				section: section,
				text:    normalize( ( card.textContent || '' ).replace( /\s+/g, ' ' ).trim() )
			} );
		} );
	} );
	...
	function filter() {
		...
		cards.forEach( function ( c ) {
			var match = c.text.indexOf( q ) !== -1;
			c.el.hidden = ! match;
			if ( match ) {
				anyVisible = true;
				if ( visibleSections.indexOf( c.section ) === -1 ) {
					visibleSections.push( c.section );
				}
			}
		} );

		sections.forEach( function ( s ) {
			s.hidden = visibleSections.indexOf( s ) === -1;
		} );
