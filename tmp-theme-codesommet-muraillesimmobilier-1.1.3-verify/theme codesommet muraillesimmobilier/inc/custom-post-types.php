...
/**
 * REST auth callback for editable property meta.
 *
 ...
 * @return bool
 */
function murailles_property_meta_auth_callback( $allowed, $meta_key, $post_id ) {
	unset( $allowed, $meta_key );
	return current_user_can( 'edit_post', (int) $post_id );
}

/**
 * Sanitize property meta before REST or admin storage.
 *
 * @param mixed  $meta_value Raw meta value.
 * @param string $meta_key Meta key.
 * @return mixed
 */
function murailles_sanitize_property_meta_value( $meta_value, $meta_key ) {
	if ( in_array( $meta_key, array( '_property_video_url', '_property_map_embed', '_seo_og_image', '_seo_canonical' ), true ) ) {
		return esc_url_raw( (string) $meta_value );
	}

	if ( '_property_agent_id' === $meta_key ) {
		return absint( $meta_value );
	}

	if ( '_property_gallery_ids' === $meta_key ) {
		$ids = array_filter( array_map( 'absint', preg_split( '/\s*,\s*/', (string) $meta_value ) ) );
		return implode( ',', $ids );
	}

	if ( '_seo_noindex' === $meta_key ) {
		return empty( $meta_value ) ? '' : '1';
	}
...
function murailles_register_property_cpt() {
	...
	foreach ( $property_meta_keys as $key => $type ) {
		register_meta( 'post', $key, array(
			...
			'single'         => true,
			'show_in_rest'   => true,
			'sanitize_callback' => 'murailles_sanitize_property_meta_value',
			'auth_callback'     => 'murailles_property_meta_auth_callback',
		) );
	}
}
...
function murailles_get_all_meta_keys() {
	return array(
		'_property_price','_property_currency','_property_price_suffix','_property_price_label',
	...
	<div class="murailles-tabs-wrap">
		...
		<div id="tab-details" class="murailles-tab-panel active">
			...
			<div class="murailles-field-row">
				<div class="murailles-field-label">Prix<small>Montant dans la devise choisie (facultatif)</small></div>
			...
			<div class="murailles-field-row">
				<div class="murailles-field-label">Devise<small>Devise de base du prix saisi</small></div>
				<div class="murailles-field-input">
					<?php
					$current_currency = $m('_property_currency');
					if ( ! $current_currency || ! array_key_exists( $current_currency, murailles_get_currencies() ) ) {
						$current_currency = murailles_default_currency();
					}
					?>
					<select name="_property_currency">
						<?php foreach ( murailles_get_currencies() as $cur_code => $cur ) : ?>
						<option value="<?php echo esc_attr( $cur_code ); ?>" <?php selected( $current_currency, $cur_code ); ?>><?php echo esc_html( $cur['label'] ); ?></option>
						<?php endforeach; ?>
					</select>
				</div>
			</div>
...
function murailles_property_admin_column_data( $column, $post_id ) {
	switch ( $column ) {
		case 'prop_price':
			$p   = murailles_parse_price_number( get_post_meta( $post_id, '_property_price', true ) );
			$cur = get_post_meta( $post_id, '_property_currency', true );
			if ( ! array_key_exists( $cur, murailles_get_currencies() ) ) {
				$cur = murailles_default_currency();
			}
			echo null !== $p ? esc_html( murailles_format_currency_amount( $p, $cur ) ) : '—';
			break;
		case 'prop_action':
...
/* ╔═══════════════════════════════════════════════════╗
   ...
   ╚═══════════════════════════════════════════════════╝
   Les biens publiés peuvent garder des métadonnées de contact propriétaire.
   Les nouvelles soumissions publiques passent désormais par le CPT privé
   `murailles_property_submission` dans inc/forms.php. */
...
/**
 * Compte les soumissions privées en attente et affiche un badge rouge sur
 * l'entrée « Soumissions biens » dans le menu admin.
 */
add_action( 'admin_menu', function () {
	...
	foreach ( $menu as $key => $item ) {
		if ( isset( $item[2] ) && $item[2] === 'edit.php?post_type=murailles_property_submission' ) {
			$menu[ $key ][0] = sprintf(
