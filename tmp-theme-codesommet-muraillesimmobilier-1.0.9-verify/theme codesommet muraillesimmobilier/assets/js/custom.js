...
$(function() {
	...
	(jQuery, window, document), $(document).ready(function() {
		if ( typeof $.fn.navigation === 'function' && $("#navigation").length ) {
			...
			// The bundled navigation plugin binds its own click/touchstart on
			// .nav-toggle that always *opens* the off-canvas (never toggles),
			// fighting our toggle handler and causing the menu to flicker /
			// reopen when closing on mobile. Detach it so our single guarded
			// toggle (further below) is the only controller.
			$('#navigation').find('.nav-toggle').off('click touchstart');
		}
	});
