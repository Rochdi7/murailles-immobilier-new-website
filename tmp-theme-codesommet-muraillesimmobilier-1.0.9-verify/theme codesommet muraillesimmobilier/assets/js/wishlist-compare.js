...
( function () {
	...
	/**
	 * Resolve a possibly-relative URL to an absolute one against the current
	 * page, so saved thumbnails never break when rendered on another route
	 * (e.g. the favoris/compare pages).
	 */
	 ...
	 * lazy-loaded srcs that aren't populated yet. Always returns an
	 * ABSOLUTE URL (relative paths break on other pages).
	 */
	function pickRealImage( card ) {
		var imgs = card.querySelectorAll( '.listing-img-wrapper img, .gtid_blog_thumb img, img' );
		for ( var i = 0; i < imgs.length; i++ ) {
			var im = imgs[ i ];
			// Skip Slick-cloned slides and decorative icons (bed/bath/pin).
			if ( im.closest( '.slick-cloned' ) ) continue;
			if ( im.closest( '.inc-fleat-icon, .listing-card-info-icon, ._list_blickes' ) ) continue;
			if ( im.width && im.width < 40 ) continue;
			// Prefer the browser-resolved (absolute) src; fall back to lazy attrs.
	...
	function fallbackThumb() {
		return 'data:image/svg+xml;utf8,' + encodeURIComponent(
			'<svg xmlns="http://www.w3.org/2000/svg" width="120" height="80" viewBox="0 0 120 80"><rect width="120" height="80" fill="#e9ecef"/><text x="60" y="44" text-anchor="middle" fill="#adb5bd" font-family="Arial" font-size="11">Aucune image</text></svg>'
		);
	}

	function renderPreviewCard( p, context ) {
		...
		// Remove (×) button — lets the user drop the item straight from the
		// modal without opening the favoris/compare page.
		var removeBtn = '';
		if ( context ) {
			removeBtn = '<button type="button" class="mwc-remove" data-mwc-remove="' + escapeHtml( context ) + '" data-id="' + escapeHtml( String( p.id ) ) + '" '
				+ 'aria-label="Retirer" title="Retirer" '
				+ 'style="position:absolute;top:8px;right:8px;width:26px;height:26px;border:0;border-radius:50%;background:#fff;color:#dc3545;box-shadow:0 1px 4px rgba(0,0,0,.15);cursor:pointer;display:inline-flex;align-items:center;justify-content:center;font-size:13px;line-height:1;z-index:2;">'
				+ '<i class="fa-solid fa-xmark" style="pointer-events:none;"></i></button>';
		}

		return '<div style="position:relative;display:flex;gap:14px;padding:14px;background:#f8f9fa;border-radius:10px;margin-bottom:10px;align-items:flex-start;border:1px solid #f0f0f0;">'
			+ removeBtn
			+ '<img src="' + thumb + '" alt="" onerror="this.onerror=null;this.src=&#39;' + fallbackThumb() + '&#39;;" style="width:96px;height:80px;object-fit:cover;border-radius:8px;flex-shrink:0;">'
	...
	function showFloatingComparePreview() {
		...
		if ( ! props.length ) {
			...
			body = props.map( function ( p ) { return renderPreviewCard( p, 'compare' ); } ).join( '' );
	...
	function showFloatingFavorisPreview() {
		...
		if ( ! props.length ) {
			...
			body = props.slice( 0, 5 ).map( function ( p ) { return renderPreviewCard( p, 'favoris' ); } ).join( '' );
			if ( props.length > 5 ) {
	...
	function buildModal( titleHtml, bodyHtml, buttons ) {
		...
		// In-modal remove (×): drop the item from its list and re-open the
		// matching preview so the modal refreshes live (no page reload).
		overlay.addEventListener( 'click', function ( e ) {
			var rm = e.target.closest( '[data-mwc-remove]' );
			if ( ! rm ) return;
			e.preventDefault();
			var ctx = rm.getAttribute( 'data-mwc-remove' );
			var id  = parseInt( rm.getAttribute( 'data-id' ), 10 );
			if ( ctx === 'favoris' ) {
				setWishlist( getWishlist().filter( function ( x ) { return x.id !== id; } ) );
				toast( I.removed_favoris || 'Retiré des favoris' );
				overlay.remove();
				showFloatingFavorisPreview();
			} else if ( ctx === 'compare' ) {
				setCompare( getCompare().filter( function ( x ) { return x.id !== id; } ) );
				toast( I.removed_compare || 'Retiré de la comparaison' );
				overlay.remove();
				showFloatingComparePreview();
			}
		} );
	...
	function showCompareReadyModal() {
		...
		var cards = props.map( function ( p ) {
			return '<div style="flex:1;text-align:center;padding:12px;background:#f8f9fa;border-radius:8px;min-width:0;">'
				+ '<img src="' + ( ( p.thumb && p.thumb.length > 4 ) ? p.thumb : fallbackThumb() ) + '" alt="" onerror="this.onerror=null;this.src=&#39;' + fallbackThumb() + '&#39;;" style="width:100%;height:100px;object-fit:cover;border-radius:6px;">'
	...
	function showReplaceModal( newSnap ) {
		...
		var newCard = '<div style="margin-bottom:16px;padding:12px;background:#fff3cd;border-radius:8px;display:flex;gap:12px;align-items:center;">'
			+ '<img src="' + ( ( newSnap.thumb && newSnap.thumb.length > 4 ) ? newSnap.thumb : fallbackThumb() ) + '" alt="" onerror="this.onerror=null;this.src=&#39;' + fallbackThumb() + '&#39;;" style="width:60px;height:60px;object-fit:cover;border-radius:6px;">'
